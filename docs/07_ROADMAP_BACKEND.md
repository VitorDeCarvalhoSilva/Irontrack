...
3. **Migrations** executando o DDL completo de `02_SCHEMA_SQLITE.md` — incluindo os patches deste documento (Seção 1: `refresh_tokens`, colunas de verificação/reset em `users`, `stagnation_alerts`, `push_subscriptions`, colunas de snapshot em `session_exercises`). Recomenda-se uma ferramenta de migration versionada (ex: Flyway) alinhada ao ecossistema Spring Boot, com um arquivo de migration por tabela/alteração, na ordem em que as tabelas são referenciadas por FK.
4. **Seed de dados de desenvolvimento**: os exercícios `STRENGTH` reais do MVP (`02_SCHEMA_SQLITE.md` §3.2, bloco de exemplos canônicos A/A.1-A.4) — nunca os exemplos de calistenia (Handstand/Planche/Muscle-Up), que são apenas demonstrativos de extensibilidade do schema.
...
3. **`AuthController`/`AuthService`** implementando: `register` (§2.1, dispara `EmailService` para verificação); `login` (§2.2, com verificação de `email_verified_at`, bloqueio de 5 falhas via Caffeine — `05_DEVOPS_E_SEGURANCA.md` §E.2, e emissão de `RefreshToken`); `refresh` (§2.4, com rotação obrigatória de token); `logout` (§2.5); `verifyEmail` (§2.6); `forgotPassword`/`resetPassword` (§2.7, com revogação de todos os `refresh_tokens` do usuário no reset).
