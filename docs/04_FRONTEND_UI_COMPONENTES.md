...
| `RegisterScreen` | `AuthStack` | Cadastro via `POST /auth/register` (§2.1). |
| `VerifyEmailScreen` | `AuthStack` | Confirma o e-mail via `GET /auth/verify-email/{token}` (§2.6) ao montar (deep link recebido por e-mail). Exibe sucesso (CTA para `LoginScreen`) ou erro (token inválido/expirado, CTA para reenviar). |
...
| `ProfileScreen` | `AppStack` (aba) | Visualização/edição de `name`/`email` (`PATCH /users/me`, §2.8 — trocar e-mail exige nova verificação), troca de senha (`POST /users/me/change-password`, §2.9), e preferências de notificação push (seletor de dias, horário, toggle — `POST`/`DELETE /users/me/push-subscription`, §7.3-§7.4, via `expo-notifications`). |
