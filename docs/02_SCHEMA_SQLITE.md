...
Este documento define de forma absoluta o esquema do banco de dados relacional **SQLite** do projeto **IronTrack**. Ele especifica o diagrama Entidade-Relacionamento (ER), as tabelas, tipos de dados, restrições de chaves, índices de performance e fornece scripts de população inicial (seed) detalhados — todos exclusivamente voltados à musculação clássica e hipertrofia (`ADR-016`, `13_ADR_LOG.md`).
...
-- 8. TABELA: exercise_set_techniques
...
-- 9. TABELA: stagnation_alerts (06_LOGICA_DE_PROGRESSAO.md §D, 07_ROADMAP_BACKEND.md §C.4)
...
-- 10. TABELA: push_subscriptions (07_ROADMAP_BACKEND.md §C.5)
...
Os `INSERT`s abaixo exemplificam como o banco de dados armazena uma sessão real de musculação, do cadastro do usuário até o registro das séries.

### 3.1. Usuário, Ciclo de Treino e Dia de Treino

```sql
-- Inserção do Usuário
INSERT INTO users (id, name, email, password_hash, created_at, updated_at) 
VALUES ('usr-9a2f-4881', 'Gabriel Silva', 'gabriel.silva@email.com', '$2a$12$JwBf6l/b4T3C1n9XW392u.H6lO69O/7S.7U7fWp', '2026-07-01T10:00:00.000Z', '2026-07-01T10:00:00.000Z');

-- Cadastro de Ciclo de Treino
INSERT INTO training_cycles (id, user_id, name, is_active, start_date, end_date, created_at)
VALUES ('cyc-4e8c-8f43', 'usr-9a2f-4881', 'Ciclo Foco Hipertrofia (ABC)', 1, '2026-07-01T00:00:00.000Z', NULL, '2026-07-01T10:05:00.000Z');

-- Cadastro de um Dia de Treino ("A - Push")
INSERT INTO training_days (id, cycle_id, name, color_code, order_index, created_at)
VALUES ('day-3a21-9d8a', 'cyc-4e8c-8f43', 'Dia A - Supino & Ombros (Push)', '#E74C3C', 1, '2026-07-01T10:10:00.000Z');
```

### 3.2. Biblioteca de Exercícios

Exercícios de musculação canônicos do MVP — o mesmo seed real usado por
`00_PRD_IRONTRACK.md` EP-02/Sprint 2 (biblioteca padrão de +50 exercícios
cobrindo peito, costas, ombros, bíceps, tríceps, pernas, glúteos e core;
os 5 abaixo são uma amostra representativa, não a lista completa).

```sql
-- Supino Reto. load_increment_kg = 2.5: exercício de empurrar de membros superiores (ajuste fino padrão).
INSERT INTO exercise_library (id, name, primary_muscle, is_custom, user_id, load_increment_kg, created_at)
VALUES ('exe-0001-bench', 'Supino Reto com Barra', 'Peito', 0, NULL, 2.5, '2026-07-01T08:00:00.000Z');

-- Demais exercícios canônicos do MVP.
...
Exemplo de uma sessão iniciada por Gabriel Silva para executar o Supino Reto.
...
-- Séries do Supino Reto
INSERT INTO exercise_sets (id, session_exercise_id, set_number, weight, reps, reps_target, rpe, notes, created_at) VALUES
('set-e1s1', 'sexe-e101', 1, 80.0, 10, 10, 8, 'Excelente controle na excêntrica', '2026-07-01T15:05:00.000Z'),
('set-e1s2', 'sexe-e101', 2, 80.0, 10, 10, 9, NULL, '2026-07-01T15:09:00.000Z'),
('set-e1s3', 'sexe-e101', 3, 80.0, 8,  10, 10, 'Falha concêntrica na 9a repetição', '2026-07-01T15:13:00.000Z');
```
...
-- Utilidade: Alimentar o gráfico de evolução de carga de um exercício específico na tela de métricas.
